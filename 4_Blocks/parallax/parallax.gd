...
func _ready() -> void: 
	TheLawsOfTheLand.perspective_changed.connect(_on_perspective_changed)
	ParallaxMesh.get_surface_override_material(0).emission = INACTIVE_COLOR
func _on_perspective_changed(Orthogonal: bool) -> void:
	...
	ParallaxMesh.get_surface_override_material(0).emission_enabled = Orthogonal
